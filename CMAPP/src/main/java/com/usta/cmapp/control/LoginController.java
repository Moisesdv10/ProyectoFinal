...
public class LoginController implements Serializable {
	...
	@EJB
	private ServicesDao<PGLoginPostgres> servicesDaoPostgres;
	...
	public String acessCheck() {
		...
		try {
			...
			if (login.getIdLoginSerial() > 0) {
				...
				PGLoginPostgres lpos = new com.covidapp_postgres.model.PGLoginPostgres();
			...
			if (login != null) {
				...
				FacesContext.getCurrentInstance()
						.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
								...
								properties.getProperty("passRecupera").concat("TU CONTRASENA ES: ").concat(password)));
			} else {
				PGLoginPostgres loginPostgres = servicesDaoPostgres.searchUser(user, EnumDataBase.POSTGRESQL.getId());
